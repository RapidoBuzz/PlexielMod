...
package net.mcreator.plexielmod.keybind;
...
import net.mcreator.plexielmod.procedures.BackpackToucheRelacheeProcedure;
...
public class BackpackKeyBinding extends PlexielmodModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	private long lastpress = 0;
	public BackpackKeyBinding(PlexielmodModElements instance) {
		super(instance, 80);
	...
	public void initElements() {
		keys = new KeyBinding("key.plexielmod.backpack", GLFW.GLFW_KEY_B, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					PlexielmodMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
		...
		if (type == 1) {
			{
				...
				BackpackToucheRelacheeProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
